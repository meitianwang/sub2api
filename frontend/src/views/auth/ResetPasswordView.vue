      ...
      <div v-if="isInvalidLink" class="space-y-6">
        <div class="rounded-xl border border-red-200 bg-red-50 p-6 dark:border-red-800/50 dark:bg-red-900/20">
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h3 class="text-lg font-semibold text-red-800 dark:text-red-200">{{ t('auth.invalidResetLink') }}</h3>
              <p class="mt-1.5 text-sm text-red-700 dark:text-red-300">{{ t('auth.invalidResetLinkHint') }}</p>
            </div>
          </div>
        </div>
      ...
      <div v-else-if="isSuccess" class="space-y-6">
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-800/50 dark:bg-emerald-900/20">
          <div class="flex flex-col items-center gap-4 text-center">
            ...
            <div>
              <h3 class="text-lg font-semibold text-emerald-800 dark:text-emerald-200">{{ t('auth.passwordResetSuccess') }}</h3>
              <p class="mt-1.5 text-sm text-emerald-700 dark:text-emerald-300">{{ t('auth.passwordResetSuccessHint') }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="text-center">
          <router-link
            to="/login"
            class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 via-blue-600 to-violet-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition-all hover:shadow-xl hover:shadow-violet-500/40 active:scale-[0.99]"
          >
